...
func SearchCVEs(db *sql.DB, opts *SearchOptions) ([]Vulnerability, error) {
	if opts == nil {
...
`
	...
	for rows.Next() {
		...
		var st StructuredTimeline
		if err := json.Unmarshal([]byte(v.Timeline), &st); err == nil {
			var exploitCount, weaponCount, pocCount int
			...
			v.PublicExploitCount = exploitCount + weaponCount + pocCount + repoCount
		} else {
			v.ReportedExploited = false
			v.PublicExploitCount = 0
		}
